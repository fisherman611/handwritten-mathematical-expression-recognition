...
def recognize_single_image(model, image_path, vocab, device, max_length=150, visualize_attention=False):
    ...
    if visualize_attention and attention_weights is not None:
        visualize_attention_maps(processed_img, attention_weights, latex_tokens, best_crop)
    
    return latex

def visualize_attention_maps(image, attention_weights, latex_tokens, best_crop, max_cols=4):
    """
...
def evaluate_model(model, test_folder, label_file, vocab, device, max_length=150, batch_size=32):
    """
    ...
    exprate_leq2 = round((correct + err1 + err2) / total, 4) if total > 0 else 0
    exprate_leq3 = round((correct + err1 + err2 + err3) / total, 4) if total > 0 else 0
...
def main():
    ...
    checkpoint_path = 'checkpoints/can_best.pth'
    mode = 'single'  # 'single' or 'evaluate'
    
    # For single mode
    image_path = 'data/CROHME/test/img/UN_125_em_565.bmp'  # Change this to your test image
    visualize = True
    ...
    if mode == 'single':
        ...
        metrics, results = evaluate_model(model, test_folder, label_file, vocab, device)
